...
newStopWords = ["0o", "0s", "3a", "3b", "3d", "6b", "6o", "a", "a1", "a2", "a3", "a4", "ab", "able", "about", "above",
...
async def sub_top_post_ids(subs, reddit):
    ...
    for posts in all_subs:
        if posts.url.endswith(".jpg") or posts.url.endswith(
                ".png") or "www.reddit.com" not in posts.url or "/comments/" not in posts.url:
            pass
        else:
            try:
                print(posts.url)
                submission = await reddit.submission(url=posts.url)
...
async def all_sub_comments(subr: list = None):
    ...
    reddit_creds = []
    for k in subr:
        reddit_creds.append(reddit)
    posts_id = await asyncio.gather(*map(sub_top_post_ids, subr, reddit_creds))
    return posts_id, reddit


if __name__ == "__main__":
    # add your subs in the list below
    subreddits = ["dnd"]
    if sys.version_info[0] == 3 and sys.version_info[1] >= 8 and sys.platform.startswith('win'):
        asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
    posts_comments, reddit = asyncio.run(all_sub_comments(subr=subreddits))
    posts_comments = flatten(posts_comments)
    df = pd.DataFrame(posts_comments, columns=['comments'])
    df['comments'] = df['comments'].str.replace(r'[^a-zA-Z0-9]+', ' ')
    df['comments'] = df.comments.str.lower()
    df['comments_without_stopwords'] = df['comments'].apply(
        lambda x: ' '.join([word for word in x.split() if word not in stop]))
    df = df[df.comments_without_stopwords.notnull()]
    df['comments_without_stopwords'] = df['comments_without_stopwords'].str.replace(r'[^a-zA-Z0-9]+', ' ')
    df['text_lemmatized'] = df.comments_without_stopwords.apply(lemmatize_text)
    df['text_lemmatized_into_string'] = [','.join(map(str, l)) for l in df['text_lemmatized']]
    text = df['text_lemmatized_into_string'].values
    ...
    plt.show()
    # plt.savefig('c.png')
    wordcloud.to_file("dnd.png")
    df.to_csv("dnd_comments.csv")
